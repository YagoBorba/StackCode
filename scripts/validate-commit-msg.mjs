...
if (fs.existsSync(configPath)) {
    try {
        ...
        if (config.features?.commitValidation === false) {
            console.log(chalk.yellow('ℹ Commit validation is disabled in .stackcoderc.json. Skipping.'));
            process.exit(0); 
        }
    } catch (e) {
        console.warn(chalk.yellow('Warning: Could not parse .stackcoderc.json. Proceeding with validation.'));
    }
}
