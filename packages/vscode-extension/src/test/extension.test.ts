...
describe("ProactiveNotificationManager Test Suite", () => {
  ...
  it("Should detect conventional commit messages correctly", () => {
    ...
    assert.strictEqual(
      manager.isConventionalCommit("feat: add new feature"),
      true,
    );
    assert.strictEqual(
      manager.isConventionalCommit("fix(auth): resolve login issue"),
      true,
    );
    assert.strictEqual(
      manager.isConventionalCommit("docs: update readme"),
      true,
    );
    assert.strictEqual(
      manager.isConventionalCommit("random commit message"),
      false,
    );
    assert.strictEqual(
      manager.isConventionalCommit("WIP: work in progress"),
      false,
    );
  });
