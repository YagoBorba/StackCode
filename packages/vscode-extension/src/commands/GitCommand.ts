    ...
    async startBranch(): Promise<void> {
        try {
            ...
            vscode.window.withProgress({
                ...
                progress.report({ increment: 50, message: 'Switching to new branch...' });
                
                await this.runTerminalCommand(command, workspaceFolder.uri.fsPath);
                
                progress.report({ increment: 100, message: 'Branch created successfully!' });
            });
    ...
    async finishBranch(): Promise<void> {
        try {
            ...
            vscode.window.withProgress({
                ...
                progress.report({ increment: 100, message: 'Branch finished successfully!' });
            });
