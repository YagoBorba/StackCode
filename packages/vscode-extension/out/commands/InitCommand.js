...
class InitCommand extends BaseCommand_1.BaseCommand {
    ...
    async execute() {
        try {
            ...
            const featureItems = [
                {
                    label: this.safeTranslate("vscode.init.features.docker.label", "Docker support"),
                    description: this.safeTranslate("vscode.init.features.docker.description", "Adds Docker configuration to the project"),
                    picked: true,
                    value: "docker",
                },
                {
                    label: this.safeTranslate("vscode.init.features.husky.label", "Husky commit hooks"),
                    description: this.safeTranslate("vscode.init.features.husky.description", "Installs Husky to enforce commit conventions"),
                    picked: true,
                    value: "husky",
                },
            ];
            const selectedFeatures = await vscode.window.showQuickPick(featureItems, {
                canPickMany: true,
                placeHolder: this.safeTranslate("init.prompt.features", "Select optional features"),
            });
            ...
            const features = (selectedFeatures.length > 0
                ? selectedFeatures
                : featureItems.filter((item) => item.picked)).map((item) => item.value);
            ...
            if (features.includes("husky")) {
                const validationChoice = await vscode.window.showQuickPick([
                    ...
                    placeHolder: this.safeTranslate("init.prompt.commit_validation", "Enable commit validation hooks?"),
                });
                if (!validationChoice) {
                    return;
                }
            ...
            const initializingTitle = this.safeTranslate("vscode.init.initializing_project", `Initializing project ${projectName}`, { projectName });
            const workflowResult = await vscode.window.withProgress({
                ...
                progress.report({
                    message: this.safeTranslate("vscode.init.setting_up_structure", "Setting up project structure..."),
                });
                const hooks = this.createWorkflowHooks(progress);
                return (0, core_1.runInitWorkflow)(workflowOptions, hooks);
            });
            if (!workflowResult) {
                return;
            }
            ...
            if (!workflowResult.dependenciesInstalled && workflowResult.installCommand) {
                ...
                const failureMessage = this.safeTranslate("init.error.deps_install_failed", `Failed to install dependencies: ${lastWarning}`, { error: lastWarning });
                const manualMessage = this.safeTranslate("init.error.deps_install_manual", "Please run the install command manually:");
                await vscode.window.showWarningMessage(`${failureMessage}\n${manualMessage}\n${installCommandString}`);
            }
            const openProjectLabel = this.safeTranslate("vscode.init.open_project", "Open Project");
            const laterLabel = this.safeTranslate("vscode.init.later", "Later");
            const successMessage = this.safeTranslate("vscode.init.project_created_successfully", `Project ${projectName} created successfully!`, { projectName });
            const openProject = await vscode.window.showInformationMessage(successMessage, openProjectLabel, laterLabel);
            ...
            this.showError((0, i18n_1.t)("vscode.init.failed_initialize_project", { error: String(error) }));
        }
    }
    /**
    ...
    createWorkflowHooks(progress) {
        const stepMessage = (step) => {
            switch (step) {
                ...
                case "saveConfig":
                    return this.safeTranslate("vscode.init.step.save_config", "Saving StackCode configuration...");
                case "generateReadme":
                ...
                case "validateDependencies":
                    return this.safeTranslate("init.step.validate_deps", "Validating local dependencies...");
                case "installDependencies":
                    return this.safeTranslate("init.step.deps", "Installing project dependencies...");
                case "completed":
                    return this.safeTranslate("vscode.init.project_initialized_successfully", "Project initialized successfully!");
                default:
                    return undefined;
            }
        };
        return {
            ...
            onMissingDependencies: async (details) => {
                await vscode.window.showWarningMessage(this.formatMissingDependenciesMessage(details));
            },
            confirmContinueAfterMissingDependencies: async () => {
                ...
                const choice = await vscode.window.showWarningMessage(this.safeTranslate("init.dependencies.prompt_continue", "Continue even if some dependencies are missing?"), { modal: true }, continueLabel, cancelLabel);
     ...
     */
    formatMissingDependenciesMessage(details) {
        const header = this.safeTranslate("init.dependencies.missing", `Missing dependencies for ${details.stack}`, { stack: details.stack });
        const missingLines = details.missingDependencies.map((dependency) => this.safeTranslate("init.dependencies.missing_detail", `  - ${dependency}`, { command: dependency }));
        const instructionHeader = this.safeTranslate("init.dependencies.install_instructions", "Install the following dependencies:");
        const installLines = details.missingDependencies.map((dependency) => this.safeTranslate(`init.dependencies.install_${dependency}`, `  - ${dependency}`));
        const optionalWarning = this.safeTranslate("init.dependencies.optional_skip", "You can skip for now, but remember to install them later.");
        return [
            header,
            ...missingLines,
            "",
     ...
     */
    safeTranslate(key, fallback, variables) {
        try {
