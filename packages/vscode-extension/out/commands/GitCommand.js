...
class GitCommand extends BaseCommand_1.BaseCommand {
    ...
    async startBranch() {
        try {
            ...
            const branchType = await vscode.window.showQuickPick([
                { label: "feature", description: (0, i18n_1.t)("vscode.git.feature_description") },
            ...
            vscode.window.withProgress({
                ...
                title: (0, i18n_1.t)("vscode.git.creating_branch", { branchName: `${branchType.label}/${branchName}` }),
                cancellable: false,
            }, async (progress) => {
                progress.report({ increment: 0, message: (0, i18n_1.t)("vscode.git.switching_to_develop") });
            ...
            this.showSuccess((0, i18n_1.t)("vscode.git.new_branch_created", { branchName: `${branchType.label}/${branchName}` }));
        }
        catch (error) {
    ...
    async finishBranch() {
        try {
            ...
            vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
                title: (0, i18n_1.t)("vscode.git.finishing_branch", { branchName: currentBranch }),
                cancellable: false,
            }, async (progress) => {
                progress.report({ increment: 0, message: (0, i18n_1.t)("vscode.git.pushing_branch") });
                // Use StackCode CLI for git operations
                const command = `npx @stackcode/cli git finish`;
                progress.report({ increment: 50, message: (0, i18n_1.t)("vscode.git.opening_pr") });
                await this.runTerminalCommand(command, workspaceFolder.uri.fsPath);
                progress.report({
                    increment: 100,
