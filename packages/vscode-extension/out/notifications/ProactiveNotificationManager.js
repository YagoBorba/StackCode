...
class ProactiveNotificationManager {
    ...
    async handleLearnMore(message) {
        ...
        const topic = Object.keys(learnMoreUrls).find((key) => message.toLowerCase().includes(key)) || "git best practices";
    ...
    async handleShowDetails(issue) {
        const outputChannel = vscode.window.createOutputChannel("StackCode Details");
        outputChannel.appendLine(`=== Project Issue Details ===`);
        outputChannel.appendLine(`Issue: ${issue}`);
        outputChannel.appendLine(`Timestamp: ${new Date().toISOString()}`);
        outputChannel.appendLine(`Workspace: ${vscode.workspace.name || "Unknown"}`);
