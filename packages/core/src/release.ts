...
async function _getLatestTags(packageNames: string[], projectRoot: string): Promise<Map<string, string>> {
...
export async function determinePackageBumps(changedPackages: PackageInfo[]): Promise<PackageBumpInfo[]> {
  const bumpInfoPromises = changedPackages.map(async (pkg) => {
    ...
    const newVersion = semver.inc(pkg.version || '0.0.0', bumpType as semver.ReleaseType);
...
export function generateChangelog(monorepoInfo: MonorepoInfo, pkgInfo?: PackageBumpInfo): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      stream.on('data', (chunk: Buffer) => (changelogContent += chunk.toString()));
...
export async function performReleaseCommit(packages: PackageBumpInfo[], projectRoot: string): Promise<void> {
