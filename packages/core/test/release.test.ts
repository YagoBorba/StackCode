...
describe('performReleaseCommit', () => {
  ...
  it('should run git add, commit, and tag in the correct order', async () => {
    ...
    expect(runCommandMock).toHaveBeenCalledWith(
      'git',
      ['add', '/fake/path/to/cli/package.json', '/fake/path/to/cli/CHANGELOG.md'],
      { cwd: projectRoot }
    );
  ...
  it('should create a consolidated commit for multiple packages', async () => {
    ...
    const mockPackages: PackageBumpInfo[] = [
      { pkg: { name: '@stackcode/cli', path: '/fake/path/to/cli' }, bumpType: 'minor', newVersion: '1.2.0' },
      { pkg: { name: '@stackcode/core', path: '/fake/path/to/core' }, bumpType: 'patch', newVersion: '1.1.1' },
    ];
    const projectRoot = '/fake/path';
